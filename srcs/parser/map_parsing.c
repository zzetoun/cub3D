...
static	bool	find_and_validate_player(t_cud *cud)
{
	...
	cud->map.pl.y = scan_identifier(cud, -1);
	cud->map.pl.facing = -1;
	while (cud->par.data[cud->map.pl.y])
	{
		line = cud->par.data[cud->map.pl.y];
		if (ft_strpbrk(line, "10"))
		{
			if (ft_strchr(line, 'N'))
				cud->map.pl.facing = N;
...
static	bool	map_pos(t_cud *cud)
{
	...
	idx = -1;
	map_id = cud->map.pl.y;
	s = cud->par.data[cud->map.pl.y];
	while (++idx < 7)
	{
		i = cud->par.id_idx[idx];
		if (i != -1 && map_id < i && ft_strpbrk(s, "10"))
